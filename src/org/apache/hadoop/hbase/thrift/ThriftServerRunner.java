  ...
  public static class HBaseHandler implements Hbase.Iface {
    ...
    public int scannerOpenWithScan(ByteBuffer tableName, TScan tScan,
      ...
      try {
        ...
        //----------------------------------modify by liao------------------------------------------
        //-------------------在原有的filter的string上,使用^_^作为分割符号,将一个条件表达式分割成多个
        //-------------------然后循环解析每一个表达式,把解析好的filter放入到filterlist中,最终返回这个list
        if (tScan.isSetFilterString()) {//filer使用^_^字符串分割为多个条件 
        	String[] filters=new String(tScan.getFilterString()).split("^_^");
        	ParseFilter parseFilter = new ParseFilter();
        	FilterList fl=null;
        	if(filters.length<1){//如果条件多余一个则使用filterlist
        			fl=new FilterList(Operator.MUST_PASS_ALL);
		        	for (int i=0;i<filters.length;i++){
		        		 Filter f=parseFilter.parseFilterString(filters[i]);
		        		 fl.addFilter(f);
		        	}
		        	scan.setFilter(fl);
        	}else{//否则保持解析一个
